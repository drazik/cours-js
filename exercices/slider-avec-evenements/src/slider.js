...
export const init = (root) => {
  const elements = getElements(root);
	...
	// Ajouter un event listener sur le bouton previous pour aller à la slide
	// précédente lorsqu'on click dessus


	// Ajouter un event listener sur le bouton next pour aller à la slide suivante
	// lorsqu'on click dessus


	// Ajouter un event listener sur *chaque* bullet pour aller à la slide
	// correspondante lorsqu'on click dessus
...
if (import.meta.vitest) {
	 ...
	 */
	const isShowingSlide = (slider, slideIndex) => {
		return slider.elements.slidesContainer.style.translate === `${-slideIndex * 100}%`
	}
	...
	describe("Lorsqu'on click sur le bouton next", () => {
		describe("Lorsqu'on n'est pas sur la dernière slide", () => {
			it.skip("La slide suivante est affichée", async () => {
				const user = userEvent.setup()

				const { slider } = setup()

				expect(isShowingSlide(slider, 0)).toBe(true)

				await user.click(slider.elements.controls.next)

				expect(isShowingSlide(slider, 1)).toBe(true)
			})
		})

		describe("Lorsqu'onn est sur la dernière slide", () => {
			it.skip("Rien ne change", async () => {
				const user = userEvent.setup()
				const { slider } = setup()
	...
	describe("Lorsqu'on click sur le bouton previous", () => {
		describe("Lorsqu'on est sur la première slide", async () => {
			it.skip("Rien ne change", async () => {
				const user = userEvent.setup()
				const { slider } = setup()
				...
				await user.click(slider.elements.controls.previous)
				expect(isShowingSlide(slider, 0)).toBe(true)
			})
		})

		describe("Lorsqu'on n'es pas sur la première slide", async () => {
			it.skip("La slide précédente est affichée", async () => {
				const user = userEvent.setup()
	...
	describe("Lorsqu'on click sur une bullet", () => {
		it.skip("La slide correspondante est affichée", async () => {
			const user = userEvent.setup()
			...
			await user.click(slider.elements.bullets[1])
			expect(isShowingSlide(slider, 1)).toBe(true)
		})
	})
}
